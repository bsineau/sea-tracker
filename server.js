  ...
  if($('mtBtn')) $('mtBtn').onclick=function(){
    ...
    api('/api/admin/meteotest').then(function(r){
      ...
      (d.essais||[]).forEach(function(e){
        var ok=e.ok||e.statut===200;
        h+='<div style="color:'+(ok?'#4ade80':'#f87171')+'">'+(ok?'✓':'✗')+' '+esc(e.nom||'')+' : '
          +esc(String(e.statut||e.code||''))+(e.type?(' · '+esc(e.type)):'')+(e.message?(' · '+esc(String(e.message).slice(0,90))):'')+'</div>';
      });
      $('mtOut').innerHTML=h;
...
const BUILD = '31/07 — test meteo console';
