...
function creerPressionIsobares(map){
  ...
  function rafraichir(force){
    ...
    if(map.getZoom() < ZOOM_MIN){ groupe.clearLayers(); veilleLegP(); return; }
...
function creerVentFleches(map){
  ...
  function rafraichir(force){
    if(!actif || !map) return;
    if(attente){ clearTimeout(attente); attente = null; }
    if(map.getZoom() < ZOOM_MIN){ groupe.clearLayers(); veilleLegV(); return; }
    if(enCours){ enAttente = true; return; }
    var reste = 700 - (Date.now() - dernier);
    if(!force && reste > 0){ planifier(reste); return; }
...
const BUILD = '01/08g — couverture pleine fenetre';
