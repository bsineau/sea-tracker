...
function creerVentFleches(map){
  ...
  function barbuleVent(lat, lon, kt, dirDe){
    ...
    var W = 44, H = 50, xh = 22, yBas = 34, yHaut = 8;
    ...
    if(n5 < 3){
      ...
      for(var i = 0; i < fanions; i++){
        corps += '<polygon points="' + xh + ',' + y + ' ' + (xh + 12) + ',' + (y + 3.5) + ' ' + xh + ',' + (y + 7)
              + '" fill="COULEUR" stroke="COULEUR" stroke-width="LARGE" stroke-linejoin="round"/>';
        y += 9;
      }
      for(var j = 0; j < pleines; j++){ corps += trait(xh, y, xh + 12, y + 4); y += 5; }
      if(demi){
        if(y === yHaut) y += 5;   /* une demi-barbule seule ne se colle pas a la pointe */
        corps += trait(xh, y, xh + 7, y + 2.3);
      }
    }
  ...
  function dessinerVent(d){
    ...
    var taille = map.getSize(), PAS = 52;
    var NX = Math.max(4, Math.min(14, Math.round(taille.x / PAS)));
    var NY = Math.max(4, Math.min(24, Math.round(taille.y / PAS)));
...
const BUILD = '01/08n — barbules meteo standard';
