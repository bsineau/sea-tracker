...
/* Motif du dernier echec Open-Meteo. omGet avalait tout : sur un 429 (quota)
   ou un 414 (URL trop longue), r.json() echoue et on renvoyait null, sans
   jamais savoir pourquoi. Le motif remonte desormais jusqu'au diagnostic. */
let omDernierMotif = '';
async function omGet(url) {
  try {
    const r = await fetch(url);
    if (!r.ok) {
      let txt = ''; try { txt = (await r.text()).slice(0, 160); } catch {}
      omDernierMotif = 'HTTP ' + r.status + (txt ? ' ' + txt.replace(/\s+/g, ' ') : '');
      return null;
    }
    const j = await r.json();
    if (j && j.error) omDernierMotif = 'API ' + (j.reason || 'erreur');
    return j;
  } catch (e) { omDernierMotif = 'reseau ' + (e && e.message ? e.message : '?'); return null; }
}

/* Cache des grilles meteo. Chaque coordonnee compte comme un appel dans le
   quota Open-Meteo : sans cache, un aller-retour de zoom en consomme plusieurs
   centaines et sature la limite par minute. Cle arrondie pour que de petits
   deplacements retombent sur la meme entree. */
const grilleCache = new Map();
function cacheLire(cle, ttlMs) {
  const e = grilleCache.get(cle);
  if (!e) return null;
  if (Date.now() - e.t > ttlMs) { grilleCache.delete(cle); return null; }
  return e.v;
}
function cacheEcrire(cle, v) {
  if (grilleCache.size > 60) { const k = grilleCache.keys().next().value; grilleCache.delete(k); }
  grilleCache.set(cle, { t: Date.now(), v: v });
}
function cacheCle() { return Array.prototype.slice.call(arguments).map(function (x) {
  return (typeof x === 'number') ? (Math.round(x * 4) / 4).toFixed(2) : String(x); }).join('|'); }
...
(function(){
  ...
  weather['Vent']=L.layerGroup([owm('wind_new',0.55),creerVentFleches(map)]); weather['Pression']=L.layerGroup([owm('pressure_new',0.25),creerPressionIsobares(map)]);
...
/* ---- Interpolation bicubique (Catmull-Rom) d'une grille reguliere ----
   Les champs meteo synoptiques sont lisses : une grille grossiere suffit a les
   decrire. On demande donc peu de points a Open-Meteo — chaque point compte
   comme un appel dans le quota, et c'est ce qui saturait la limite par minute —
   puis on redensifie ici pour l'affichage. Catmull-Rom plutot que bilineaire :
   cette derniere laisse des cassures visibles sur les isobares. */
function crInterp(p0, p1, p2, p3, t){
  return 0.5 * ((2 * p1) + (-p0 + p2) * t
    + (2 * p0 - 5 * p1 + 4 * p2 - p3) * t * t
    + (-p0 + 3 * p1 - 3 * p2 + p3) * t * t * t);
}
function grilleEch(g, nx, ny, x, y){
  x = x < 0 ? 0 : (x > nx - 1 ? nx - 1 : x);
  y = y < 0 ? 0 : (y > ny - 1 ? ny - 1 : y);
  var v = g[y * nx + x];
  return (v === null || v === undefined || !isFinite(v)) ? 0 : v;
}
function interpolerGrille(g, nx, ny, NX, NY){
  if(nx < 2 || ny < 2) return null;
  var out = new Array(NX * NY);
  for(var j = 0; j < NY; j++){
    var fy = (ny - 1) * j / (NY - 1), y0 = Math.floor(fy), ty = fy - y0;
    for(var i = 0; i < NX; i++){
      var fx = (nx - 1) * i / (NX - 1), x0 = Math.floor(fx), tx = fx - x0;
      var c = [0, 0, 0, 0];
      for(var m = -1; m <= 2; m++){
        c[m + 1] = crInterp(grilleEch(g, nx, ny, x0 - 1, y0 + m), grilleEch(g, nx, ny, x0, y0 + m),
                            grilleEch(g, nx, ny, x0 + 1, y0 + m), grilleEch(g, nx, ny, x0 + 2, y0 + m), tx);
      }
      out[j * NX + i] = crInterp(c[0], c[1], c[2], c[3], ty);
    }
  }
  return out;
}

/* ---- Calque « Pression » : isobares + centres A / D ----
   La nappe OpenWeather ne porte ni ligne ni valeur : elle situe vaguement les
   zones mais ne permet pas de lire un gradient. On trace donc les isobares a
   partir d'une grille Open-Meteo (pression au niveau de la mer), par la
   methode des carres marchants : pour chaque maille traversee par un niveau,
   on interpole le point de passage sur chaque arete. Les segments partagent
   leurs extremites, la ligne parait donc continue. Les cas ambigus (deux
   diagonales possibles) sont tranches par la moyenne des quatre coins.
   Les centres sont les extremums locaux de la grille. */
function creerPressionIsobares(map){
  var groupe = L.layerGroup();
  var timer = null, dernier = 0, enCours = false, actif = false;
  var attente = null, enAttente = false;
  var ZOOM_MIN = 2, pasHpa = 4;

  function interp(v1, v2, niveau){
    var d = v2 - v1;
    return (Math.abs(d) < 1e-9) ? 0.5 : (niveau - v1) / d;
  }

  /* segments d'un niveau donne, en coordonnees geographiques */
  function segments(g, nx, ny, la0, dA, lo0, dO, niveau){
    var out = [];
    for(var iy = 0; iy < ny - 1; iy++){
      for(var ix = 0; ix < nx - 1; ix++){
        var a = g[iy * nx + ix], b = g[iy * nx + ix + 1];
        var c = g[(iy + 1) * nx + ix + 1], d = g[(iy + 1) * nx + ix];
        if(a === null || b === null || c === null || d === null) continue;
        var idx = (a >= niveau ? 8 : 0) | (b >= niveau ? 4 : 0) | (c >= niveau ? 2 : 0) | (d >= niveau ? 1 : 0);
        if(idx === 0 || idx === 15) continue;
        var laH = la0 + iy * dA, laB = la0 + (iy + 1) * dA;
        var loG = lo0 + ix * dO, loD = lo0 + (ix + 1) * dO;
        var pHaut  = [laH, loG + interp(a, b, niveau) * dO];
        var pDroit = [laH + interp(b, c, niveau) * dA, loD];
        var pBas   = [laB, loG + interp(d, c, niveau) * dO];
        var pGauche= [laH + interp(a, d, niveau) * dA, loG];
        var moy = (a + b + c + d) / 4;
        if(idx === 5){ idx = (moy >= niveau) ? 51 : 52; }
        else if(idx === 10){ idx = (moy >= niveau) ? 101 : 102; }
        switch(idx){
          case 1: case 14: out.push([pGauche, pBas]); break;
          case 2: case 13: out.push([pBas, pDroit]); break;
          case 3: case 12: out.push([pGauche, pDroit]); break;
          case 4: case 11: out.push([pHaut, pDroit]); break;
          case 6: case 9:  out.push([pHaut, pBas]); break;
          case 7: case 8:  out.push([pHaut, pGauche]); break;
          case 51: out.push([pHaut, pGauche]); out.push([pBas, pDroit]); break;
          case 52: out.push([pHaut, pDroit]); out.push([pGauche, pBas]); break;
          case 101: out.push([pHaut, pDroit]); out.push([pGauche, pBas]); break;
          case 102: out.push([pHaut, pGauche]); out.push([pBas, pDroit]); break;
        }
      }
    }
    return out;
  ...
  /* Rampe de couleur relative : la nappe OpenWeather utilise une echelle
     absolue (~950-1050 hPa), illisible sur une fenetre ou la pression ne varie
     que de quelques hPa. Celle-ci est recalculee sur les extremes de la
     fenetre affichee, donc contrastee a toute echelle. */
  /* Rampe inspiree des cartes synoptiques : turquoise pour les bas, teinte
     neutre au centre, brun soutenu pour les hauts. Plus contrastee que la
     precedente, qui virait au jaune uniforme sur une large fenetre. */
  var RAMPE = [[0.00,12,94,130],[0.18,40,160,178],[0.36,150,214,214],[0.50,238,238,228],
               [0.64,222,186,128],[0.82,196,124,58],[1.00,140,62,30]];
  function rampe(t){
    t = t < 0 ? 0 : (t > 1 ? 1 : t);
    for(var i = 1; i < RAMPE.length; i++){
      if(t <= RAMPE[i][0]){
        var a = RAMPE[i-1], b = RAMPE[i];
        var u = (t - a[0]) / (b[0] - a[0]);
  ...
  /* Peinte dans un canvas a la maille de la grille, puis etiree par le
     navigateur : le lissage bilineaire donne un degrade continu sans qu'on ait
     a interpoler nous-memes. Isolee : en cas d'echec, seules les isobares
     restent (R3). */
  function nappeRelative(d, mini, maxi){
    try{
      var cv = document.createElement('canvas');
      cv.width = d.nx; cv.height = d.ny;
      var ctx = cv.getContext('2d'); if(!ctx) return null;
      var im = ctx.createImageData(d.nx, d.ny), etendue = Math.max(0.5, maxi - mini);
      for(var y = 0; y < d.ny; y++){
        for(var x = 0; x < d.nx; x++){
          var v = d.valeurs[y * d.nx + x];
          var px = ((d.ny - 1 - y) * d.nx + x) * 4;   /* canvas : y=0 en haut */
          if(v === null || v === undefined){ im.data[px+3] = 0; continue; }
          var c = rampe((v - mini) / etendue);
          im.data[px] = c[0]; im.data[px+1] = c[1]; im.data[px+2] = c[2]; im.data[px+3] = 255;
        }
      }
      ctx.putImageData(im, 0, 0);
      return L.imageOverlay(cv.toDataURL(), [[d.lat0, d.lon0], [d.lat1, d.lon1]],
                            { opacity:0.62, interactive:false });
    }catch(e){ return null; }
  }

  function etiquette(lat, lon, texte, gras){
    var css = 'font:' + (gras ? '700 11px' : '600 10px') + '/1 -apple-system,system-ui,sans-serif;'
      + 'color:#10202c;background:rgba(255,255,255,.86);border-radius:3px;padding:1px 3px;white-space:nowrap';
    return L.marker([lat, lon], {
      icon: L.divIcon({ className:'isobare-val', html:'<div style="' + css + '">' + texte + '</div>', iconSize:[34,14], iconAnchor:[17,7] }),
      interactive:false, keyboard:false
    });
  }

  function centre(lat, lon, type, val){
    var c = (type === 'A') ? '#b91c1c' : '#1d4ed8';
    var html = '<div style="text-align:center;line-height:1">'
      + '<div style="font:800 15px/1 -apple-system,system-ui,sans-serif;color:' + c + ';'
      + 'text-shadow:0 0 4px #fff,0 0 4px #fff,0 0 4px #fff">' + type + '</div>'
      + '<div style="font:700 10px/1.2 -apple-system,system-ui,sans-serif;color:#10202c;'
      + 'text-shadow:0 0 3px #fff,0 0 3px #fff,0 0 3px #fff">' + Math.round(val) + '</div></div>';
    return L.marker([lat, lon], {
      icon: L.divIcon({ className:'pression-centre', html:html, iconSize:[38,34], iconAnchor:[19,17] }),
      interactive:false, keyboard:false
    });
  }
  ...
  function dessiner(d){
    groupe.clearLayers();
    var g = d.valeurs, nx = d.nx, ny = d.ny;
    if(!g || nx < 2 || ny < 2) return;
    /* redensification : isobares, nappe et centres travaillent sur la grille fine */
    var FACT = 5;
    var NXf = (nx - 1) * FACT + 1, NYf = (ny - 1) * FACT + 1;
    var gf = interpolerGrille(g, nx, ny, NXf, NYf);
    if(gf){ g = gf; nx = NXf; ny = NYf; }
    var dA = (d.lat1 - d.lat0) / (ny - 1), dO = (d.lon1 - d.lon0) / (nx - 1);
    var mini = Infinity, maxi = -Infinity;
    for(var i = 0; i < g.length; i++){ if(g[i] === null) continue; if(g[i] < mini) mini = g[i]; if(g[i] > maxi) maxi = g[i]; }
    if(!isFinite(mini) || !isFinite(maxi)) return;
    /* la nappe va sous les isobares : ajoutee en premier */
    var fond = nappeRelative({ nx: nx, ny: ny, valeurs: g, lat0: d.lat0, lat1: d.lat1, lon0: d.lon0, lon1: d.lon1 }, mini, maxi);
    if(fond) fond.addTo(groupe);
    /* seuil d'ecart entre deux etiquettes d'un meme niveau : un sixieme de la
       diagonale de la fenetre, sinon elles se collent sur les lignes courtes */
    var ecartMin = (Math.abs(d.lat1 - d.lat0) + Math.abs(d.lon1 - d.lon0)) / 12;
    /* pas adapte a l'amplitude : 4 hPa laissait trop peu de lignes sur une
       fenetre calme, et en aurait trop sur un champ tres creuse */
    var amp = maxi - mini;
    /* On vise une quinzaine d'isobares quelle que soit l'echelle, en pas ronds,
       au lieu de seuils d'amplitude qui donnaient 8 hPa — et donc trop peu de
       lignes — des qu'un anticyclone et une depression etaient dans la vue. */
    var PAS_RONDS = [1, 2, 4, 5, 8, 10, 20];
    pasHpa = PAS_RONDS[PAS_RONDS.length - 1];
    for(var ip = 0; ip < PAS_RONDS.length; ip++){
      if(amp / PAS_RONDS[ip] <= 18){ pasHpa = PAS_RONDS[ip]; break; }
    }
    majLegP(mini, maxi);   /* apres le calcul du pas : la legende l'affiche */
    var n0 = Math.ceil(mini / pasHpa) * pasHpa;
    for(var niv = n0; niv <= maxi; niv += pasHpa){
      var segs = segments(g, nx, ny, d.lat0, dA, d.lon0, dO, niv);
      if(!segs.length) continue;
      var pivot = (niv % (pasHpa * 5) === 0);   /* un trait appuye toutes les 5 lignes */
      /* halo clair dessous, trait sombre dessus : lisible sur nappe coloree */
      L.polyline(segs, { color:'#ffffff', weight: pivot ? 5.5 : 4, opacity:.8, interactive:false }).addTo(groupe);
      L.polyline(segs, { color: pivot ? '#0a1620' : '#1c3span'.replace('span','a4a'),
                         weight: pivot ? 2.9 : 1.9, opacity:1, interactive:false }).addTo(groupe);
      /* une etiquette par niveau, sur un segment pris au milieu de la serie */
      var m = segs[Math.floor(segs.length / 2)];
      etiquette(m[0][0], m[0][1], String(niv), pivot).addTo(groupe);
      if(segs.length > 12){
        var q = segs[Math.floor(segs.length / 6)];
        var dl = Math.abs(q[0][0] - m[0][0]) + Math.abs(q[0][1] - m[0][1]);
        if(dl > ecartMin) etiquette(q[0][0], q[0][1], String(niv), pivot).addTo(groupe);
      }
    }
    /* Centres. Un anticyclone est plat en son sommet : exiger un ecart entre
       voisins immediats ne trouve rien (verifie, aucun centre ne sortait). On
       teste donc l'extremum au sens large sur les 8 voisins, puis on exige un
       contraste d'au moins 1 hPa sur un rayon de 2 mailles, et on ecarte les
       doublons de plateau. */
    var trouves = [];
    for(var y = 1; y < ny - 1; y++){
      for(var x = 1; x < nx - 1; x++){
        var v = g[y * nx + x]; if(v === null) continue;
        var haut = true, bas = true, ok = true;
        for(var dy = -1; dy <= 1; dy++) for(var dx = -1; dx <= 1; dx++){
          if(!dx && !dy) continue;
          var w = g[(y + dy) * nx + (x + dx)];
          if(w === null){ ok = false; continue; }
          if(w > v) haut = false;
          if(w < v) bas = false;
        }
        if(!ok || (!haut && !bas)) continue;
        /* rayon exprime en mailles fines : il doit valoir 2 mailles de la
           grille d'origine, sinon l'interpolation le rend trop court et plus
           aucun centre ne passe le critere de contraste */
        var RAY = 2 * FACT;
        var mn = v, mx = v;
        for(var ey = -RAY; ey <= RAY; ey++) for(var ex = -RAY; ex <= RAY; ex++){
          var yy = y + ey, xx = x + ex;
          if(yy < 0 || yy >= ny || xx < 0 || xx >= nx) continue;
          var uu = g[yy * nx + xx]; if(uu === null) continue;
          if(uu < mn) mn = uu; if(uu > mx) mx = uu;
        }
        var contraste = haut ? (v - mn) : (mx - v);
        if(contraste < 1) continue;
        trouves.push({ lat: d.lat0 + y * dA, lon: d.lon0 + x * dO, type: haut ? 'A' : 'D',
                       val: v, contraste: contraste, gx: x, gy: y });
      }
    }
    trouves.sort(function(p, q2){ return q2.contraste - p.contraste; });
    var retenus = [];
    for(var k = 0; k < trouves.length && retenus.length < 8; k++){
      var t = trouves[k], proche = false;
      for(var r = 0; r < retenus.length; r++)
        if(Math.abs(retenus[r].gx - t.gx) <= 2 * FACT && Math.abs(retenus[r].gy - t.gy) <= 2 * FACT){ proche = true; break; }
      if(proche) continue;
      retenus.push(t);
      centre(t.lat, t.lon, t.type, t.val).addTo(groupe);
    }
  }


  /* Une demande de rafraichissement n'est jamais abandonnee : si une requete
     est en vol ou si la retenue n'est pas ecoulee, elle est reprogrammee.
     Auparavant elle etait simplement ignoree, et la carte gardait la grille de
     l'ancienne fenetre jusqu'au timer periodique (defaut constate au dezoom). */
  function planifier(delai){
    if(attente) clearTimeout(attente);
    attente = setTimeout(function(){ attente = null; rafraichir(true); }, delai);
  }
  function rafraichir(force){
    if(!actif || !map) return;
    if(attente){ clearTimeout(attente); attente = null; }
    if(map.getZoom() < ZOOM_MIN){ groupe.clearLayers(); veilleLegP();
      window.__dgPres = { h: new Date().toTimeString().slice(0,8), zoom: (+map.getZoom()).toFixed(1),
        zone: '-', maille: '-', etat: 'en veille (zoom < ' + ZOOM_MIN + ')' };
      return; }
    if(enCours){ enAttente = true; return; }
    var reste = 700 - (Date.now() - dernier);
    if(!force && reste > 0){ planifier(reste); return; }
    var b = map.getBounds();
    var la0 = b.getSouth(), la1 = b.getNorth(), lo0 = b.getWest(), lo1 = b.getEast();
    if(la0 < -85) la0 = -85;
    if(la1 > 85) la1 = 85;
    if(lo1 - lo0 > 240){ var co = (lo0 + lo1) / 2; lo0 = co - 120; lo1 = co + 120; }
    /* Apres plusieurs zooms et deplacements, Leaflet rend des longitudes hors
       de [-180, 180] (la carte se repete) ; Open-Meteo les refuse et la requete
       echoue en silence, laissant le calque vide. On les ramene ici. */
    if(lo1 - lo0 >= 359.9){ lo0 = -180; lo1 = 180; }
    else {
      var ce = (lo0 + lo1) / 2, demi = (lo1 - lo0) / 2;
      ce = ((ce + 180) % 360 + 360) % 360 - 180;
      lo0 = ce - demi; lo1 = ce + demi;
      if(lo0 < -180){ lo0 = -180; lo1 = Math.min(180, lo0 + 2 * demi); }
      if(lo1 > 180){ lo1 = 180; lo0 = Math.max(-180, lo1 - 2 * demi); }
    }
    ...
    /* Marge de couverture : on charge un peu plus large que l'ecran, et tant
       ...
       d'utile mais consommaient le quota a chaque geste. */
    var mLa = (la1 - la0) * 0.18, mLo = (lo1 - lo0) * 0.18;
    la0 -= mLa; la1 += mLa; lo0 -= mLo; lo1 += mLo;
    /* re-bornage APRES elargissement : sinon la marge fait sortir des limites
       acceptees par l'endpoint sur les grandes fenetres */
    if(la0 < -85) la0 = -85;
    if(la1 > 85) la1 = 85;
    if(lo1 - lo0 > 240){ var cc = (lo0 + lo1) / 2; lo0 = cc - 120; lo1 = cc + 120; }
    if(lo0 < -180){ lo1 += (-180 - lo0); lo0 = -180; if(lo1 > 180) lo1 = 180; }
    if(lo1 > 180){ lo0 -= (lo1 - 180); lo1 = 180; if(lo0 < -180) lo0 = -180; }
    /* Maille calee sur des mesures d'erreur d'interpolation : viser ~5 deg en
       longitude et ~4 en latitude garde l'ecart sous 0,5 hPa sur un champ
       synoptique, tout en restant fin en cotier. */
    var nx = Math.max(6, Math.min(12, Math.round((lo1 - lo0) / 5) + 1));
    var ny = Math.max(6, Math.min(14, Math.round((la1 - la0) / 4) + 1));
    while(nx * ny > 120){ if(ny >= nx) ny--; else nx--; }
    enCours = true;
    window.__dgPres = { h: new Date().toTimeString().slice(0,8), zoom: (+map.getZoom()).toFixed(1),
      zone: la0.toFixed(1) + '/' + la1.toFixed(1) + ' ' + lo0.toFixed(1) + '/' + lo1.toFixed(1),
    ...
    fetch('/api/pression/champ?lat0=' + la0.toFixed(3) + '&lat1=' + la1.toFixed(3)
      ...
      .then(function(d){
        ...
        if(enAttente){ enAttente = false; planifier(120); }
        if(window.__dgPres) window.__dgPres.etat = (d && d.error) ? ('ERREUR ' + d.error + ' [' + (d.detail || '?') + ', ' + (d.points_demandes || '?') + ' pts]')
          : ((d && d.valeurs) ? (d.valeurs.length + ' valeurs') : 'reponse vide');
        if(!actif) return;
      ...
      .catch(function(){ enCours = false; if(enAttente){ enAttente = false; planifier(1500); } });
  }
  ...
  function dansCouverture(a0, a1, o0, o1, mod, hr){
    ...
    /* Seuil de fraicheur d'echelle. A 0,5 on tolerait une grille deux fois trop
       ...
       plusieurs centaines de milles sur une vue regionale. On recharge des que
       la vue a retreci de plus de 30 % dans un sens ou dans l'autre. */
    return (a1 - a0) / (couv.la1 - couv.la0) > 0.7
        && (o1 - o0) / (couv.lo1 - couv.lo0) > 0.7;
  }
  var legP = null, elP = null;
  function barreRampe(){
    var h = '';
    for(var i = 0; i <= 10; i++){ var c = rampe(i / 10); h += '<div style="flex:1;background:rgb(' + c[0] + ',' + c[1] + ',' + c[2] + ')"></div>'; }
    return '<div style="display:flex;height:6px;width:104px;border-radius:3px;overflow:hidden">' + h + '</div>';
  }
  function majLegP(mini, maxi){
    if(!elP) return;
    elP.innerHTML = '<div style="font-weight:700;margin-bottom:3px">Pression (hPa)</div>'
      + barreRampe()
      + '<div style="display:flex;justify-content:space-between;width:104px;font-size:9px;color:#8fb0c4;margin-bottom:2px">'
      + '<span>' + Math.round(mini) + '</span><span>' + Math.round(maxi) + '</span></div>'
      + '<div style="font-size:10px">isobares /' + pasHpa + ' hPa &middot; '
      + '<span style="color:#ef4444;font-weight:800">A</span> &middot; '
      + '<span style="color:#60a5fa;font-weight:800">D</span></div>';
  }
  function echecLegP(){
    if(elP) elP.innerHTML = '<div style="font-weight:700;margin-bottom:2px">Pression (hPa)</div>'
      + '<div style="font-size:10px;color:#fbbf24">donnees indisponibles &mdash; trace efface</div>';
  }
  function veilleLegP(){
    if(elP) elP.innerHTML = '<div style="font-weight:700;margin-bottom:2px">Pression (hPa)</div>'
      + '<div style="font-size:10px;color:#8fb0c4">isobares a partir du zoom 2</div>';
  }
  function montrerLegP(){
    if(legP) return;
    legP = L.control({position:'bottomleft'});
    legP.onAdd = function(){
      elP = L.DomUtil.create('div');
      elP.style.cssText = 'background:#0d1f2dee;color:#dfeaf2;border:1px solid #1d3a52;border-radius:10px;padding:6px 8px;font-size:11px;line-height:1.35;box-shadow:0 2px 8px rgba(0,0,0,.35)';
      veilleLegP();
      return elP;
    };
    legP.addTo(map);
  }
  function cacherLegP(){ if(legP){ try{ map.removeControl(legP); }catch(e){} legP = null; elP = null; } }
  ...
  groupe.on('remove', function(){
    actif = false; enAttente = false; couv = null; cacherLegP(); groupe.clearLayers();
    map.off('moveend', onMoveP);
    if(timer){ clearInterval(timer); timer = null; }
    if(attente){ clearTimeout(attente); attente = null; }
  });
  function onMoveP(){ rafraichir(false); }
  return groupe;
}
...
function creerVentFleches(map){
  ...
  /* Barbules a la convention meteo standard. Deux differences avec les fleches
     precedentes, voulues : la hampe pointe vers la PROVENANCE du vent (et non
     dans le sens du flux), et la vitesse est codee par arrondi au multiple de
     5 kt — demi-barbule 5, barbule pleine 10, fanion 50, cercle sous 3 kt.
     Barbules a gauche du vecteur vent dans l'hemisphere nord, donc tracees a
     droite d'une hampe dessinee vers la provenance ; miroir au sud. La valeur
     chiffree est conservee, plus discrete : sur un telephone, lire « 35 » va
     plus vite que compter trois barbules et demie. */
  function barbuleVent(lat, lon, kt, dirDe){
    var c = couleurVent(kt);
    var n5 = Math.round(kt / 5) * 5;
    var W = 36, H = 44, xh = 18, yBas = 30, yHaut = 6;
    function trait(x1, y1, x2, y2){
      return '<line x1="' + x1 + '" y1="' + y1 + '" x2="' + x2 + '" y2="' + y2
        + '" stroke="COULEUR" stroke-width="LARGE" stroke-linecap="round"/>';
    }
    var corps = '';
    if(n5 < 3){
      corps = '<circle cx="' + xh + '" cy="' + yBas + '" r="4.5" fill="none" stroke="COULEUR" stroke-width="LARGE"/>';
    } else {
      corps += trait(xh, yBas, xh, yHaut);
      var y = yHaut, reste = n5;
      var fanions = Math.floor(reste / 50); reste -= fanions * 50;
      var pleines = Math.floor(reste / 10); reste -= pleines * 10;
      var demi = (reste >= 5) ? 1 : 0;
      for(var i = 0; i < fanions; i++){
        corps += '<polygon points="' + xh + ',' + y + ' ' + (xh + 10) + ',' + (y + 3) + ' ' + xh + ',' + (y + 6)
              + '" fill="COULEUR" stroke="COULEUR" stroke-width="LARGE" stroke-linejoin="round"/>';
        y += 7.5;
      }
      for(var j = 0; j < pleines; j++){ corps += trait(xh, y, xh + 10, y + 3.4); y += 4.3; }
      if(demi){
        if(y === yHaut) y += 4.3;   /* une demi-barbule seule ne se colle pas a la pointe */
        corps += trait(xh, y, xh + 6, y + 2);
      }
    }
    var halo = corps.split('COULEUR').join('rgba(255,255,255,.85)').split('LARGE').join('4.6');
    var dessin = corps.split('COULEUR').join(c).split('LARGE').join('2.1');
    var miroir = (lat < 0) ? ' transform="translate(' + (2 * xh) + ',0) scale(-1,1)"' : '';
    var svg = '<svg width="' + W + '" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '">'
      + '<g' + miroir + '>' + halo + dessin + '</g></svg>';
    var html = '<div style="width:' + W + 'px;height:' + H + 'px;position:relative">'
      + '<div style="position:absolute;left:0;top:0;transform:rotate(' + dirDe.toFixed(1) + 'deg);'
      + 'transform-origin:' + xh + 'px ' + yBas + 'px">' + svg + '</div>'
      + '<div style="position:absolute;left:0;top:' + (yBas + 3) + 'px;width:' + W + 'px;text-align:center;'
      + 'font:600 9.5px/1 -apple-system,system-ui,sans-serif;color:#25384a;opacity:.85;'
      + 'text-shadow:0 0 3px #fff,0 0 3px #fff">' + Math.round(kt) + '</div>'
      + '</div>';
    return L.marker([lat, lon], {
      icon: L.divIcon({ className:'vent-barbule', html: html, iconSize:[W, H], iconAnchor:[xh, yBas] }),
      interactive: false, keyboard: false
    });
  }
  ...
  /* La direction ne s'interpole pas en degres (saut 359 -> 0) : on passe par
     les composantes du vecteur, on interpole, puis on recompose. La vitesse
  ...
  function dessinerVent(d){
    var n = d.nx, m = d.ny;
    if(!n || !m || d.points.length < n * m) return;
    var U = new Array(n * m), V = new Array(n * m);
    for(var i = 0; i < n * m; i++){
      var p = d.points[i], kt = p[2], a = p[3] * Math.PI / 180;
      U[i] = kt * Math.sin(a); V[i] = kt * Math.cos(a);
    }
    /* Maille d'affichage visee en milles (10 NM), et non en pixels : en cotier
       zoome on l'atteint reellement, en vue large on plafonne pour ne pas
       submerger la carte de marqueurs. Ces barbules intermediaires sont
       INTERPOLEES : elles ne coutent aucun appel, mais elles n'apportent pas
       d'information que le modele ne contient pas — d'ou la maille reelle
       affichee dans la legende. */
    var latMoy = (d.lat0 + d.lat1) / 2;
    var kNM = 60 * Math.cos(latMoy * Math.PI / 180);
    var largeurNM = (d.lon1 - d.lon0) * kNM, hauteurNM = (d.lat1 - d.lat0) * 60;
    var CIBLE_NM = 10;
    var NX = Math.max(4, Math.min(20, Math.round(largeurNM / CIBLE_NM) + 1));
    var NY = Math.max(4, Math.min(34, Math.round(hauteurNM / CIBLE_NM) + 1));
    mailleReelleNM = (d.lon1 - d.lon0) / (d.nx - 1) * kNM;
    mailleAfficheeNM = (d.lon1 - d.lon0) / (NX - 1) * kNM;
    var Uf = interpolerGrille(U, n, m, NX, NY), Vf = interpolerGrille(V, n, m, NX, NY);
    if(!Uf || !Vf) return;
    var dA = (d.lat1 - d.lat0) / (NY - 1), dO = (d.lon1 - d.lon0) / (NX - 1);
    /* la zone chargee deborde de l'ecran : inutile de poser des marqueurs
       invisibles, ils coutent en fluidite sur telephone */
    var bv = map.getBounds();
    var vs = bv.getSouth() - 0.05 * (bv.getNorth() - bv.getSouth());
    var vn = bv.getNorth() + 0.05 * (bv.getNorth() - bv.getSouth());
    var vo = bv.getWest() - 0.05 * (bv.getEast() - bv.getWest());
    var ve = bv.getEast() + 0.05 * (bv.getEast() - bv.getWest());
    var poses = 0;
    for(var y = 0; y < NY; y++){
      for(var x = 0; x < NX; x++){
        var la = d.lat0 + y * dA, lo = d.lon0 + x * dO;
        if(la < vs || la > vn || lo < vo || lo > ve) continue;
        var u = Uf[y * NX + x], v = Vf[y * NX + x];
        var kt2 = Math.sqrt(u * u + v * v);
        var dir2 = (Math.atan2(u, v) * 180 / Math.PI + 360) % 360;
        barbuleVent(la, lo, kt2, dir2).addTo(groupe);
        poses++;
      }
    }
    if(elV) elV.innerHTML = corpsLegV();
    if(window.__dgVent) window.__dgVent.affichage = poses + ' barbules, maille reelle '
      + Math.round(mailleReelleNM) + ' NM';
  }

  function rafraichir(force){
    if(!actif || !map) return;
    if(attente){ clearTimeout(attente); attente = null; }
    if(map.getZoom() < ZOOM_MIN){ groupe.clearLayers(); veilleLegV();
      window.__dgVent = { h: new Date().toTimeString().slice(0,8), zoom: (+map.getZoom()).toFixed(1),
        zone: '-', maille: '-', etat: 'en veille (zoom < ' + ZOOM_MIN + ')' };
      return; }
    if(enCours){ enAttente = true; return; }
    var reste = 700 - (Date.now() - dernier);
    ...
    var b = map.getBounds();
    var la0 = b.getSouth(), la1 = b.getNorth(), lo0 = b.getWest(), lo1 = b.getEast();
    /* On couvre TOUTE la fenetre visible : la maille s'elargit avec l'echelle,
       le nombre de fleches restant fixe par la taille de l'ecran. Auparavant on
       reduisait la zone demandee, ce qui tassait toutes les fleches dans un
       pave central en laissant le reste de la carte vide. */
    if(la0 < -85) la0 = -85;
    if(la1 > 85) la1 = 85;
    if(lo1 - lo0 > 240){ var co = (lo0 + lo1) / 2; lo0 = co - 120; lo1 = co + 120; }
    /* Apres plusieurs zooms et deplacements, Leaflet rend des longitudes hors
    ...
    fetch('/api/vent/champ?lat0=' + la0.toFixed(3) + '&lat1=' + la1.toFixed(3)
      ...
      .catch(function(e){ enCours = false;
        if(window.__dgVent) window.__dgVent.etat = 'ECHEC RESEAU';
        if(enAttente){ enAttente = false; planifier(1500); } });
  }

  var legendeV = null, elV = null;
  var mailleReelleNM = 0, mailleAfficheeNM = 0;
  /* legende compacte : une barre au lieu de sept lignes — avec la pression
     affichee, deux legendes empilees mangeaient le quart de l'ecran */
  function corpsLegV(){
    var n = ['#94a3b8','#38bdf8','#0284c7','#16a34a','#eab308','#ea580c','#dc2626'], h = '';
    for(var i=0;i<n.length;i++) h += '<div style="flex:1;background:'+n[i]+'"></div>';
    return '<div style="font-weight:700;margin-bottom:3px">Vent (kt)</div>'
      + '<div style="display:flex;height:6px;width:104px;border-radius:3px;overflow:hidden">' + h + '</div>'
      + '<div style="display:flex;justify-content:space-between;width:104px;font-size:9px;color:#8fb0c4">'
      + '<span>0</span><span>15</span><span>30+</span></div>'
      + '<div style="font-size:10px;color:#8fb0c4">barbules : provenance<br>demi 5 &middot; pleine 10 &middot; fanion 50</div>'
      + (mailleReelleNM ? ('<div style="font-size:9.5px;color:#c9a227;margin-top:2px">donnees ~'
          + Math.round(mailleReelleNM) + ' NM &middot; affichage ' + Math.round(mailleAfficheeNM)
          + ' NM (interpole)</div>') : '');
  }
  function veilleLegV(){
    if(elV) elV.innerHTML = '<div style="font-weight:700;margin-bottom:2px">Vent (kt)</div>'
      + '<div style="font-size:10px;color:#8fb0c4">barbules a partir du zoom 2</div>';
  }
  function montrerLegendeV(){
    if(legendeV) return;
    legendeV = L.control({position:'bottomleft'});
    legendeV.onAdd = function(){
      elV = L.DomUtil.create('div');
      elV.style.cssText = 'background:#0d1f2dee;color:#dfeaf2;border:1px solid #1d3a52;border-radius:10px;padding:6px 8px;font-size:11px;line-height:1.35;box-shadow:0 2px 8px rgba(0,0,0,.35)';
      elV.innerHTML = corpsLegV();
      return elV;
    };
    legendeV.addTo(map);
  }
  function cacherLegendeV(){ if(legendeV){ try{ map.removeControl(legendeV); }catch(e){} legendeV = null; elV = null; } }
  ...
  groupe.on('remove', function(){
    actif = false; enAttente = false; couv = null; cacherLegendeV(); groupe.clearLayers();
    ...
    try{
      ...
      if(typeof weather!=='undefined'){
        ...
        noms.forEach(function(n){
          ...
          if(sur){
            /* la cle ne doit pas apparaitre : sa presence se verifie a sa
               longueur, pas a sa valeur (elle finissait sur les captures) */
            var urlSure=String(url).replace(/appid=([^&]*)/, function(m,k){
              return 'appid=' + (k ? ('<'+k.length+' car. masques>') : 'ABSENTE'); });
            l('    url = '+urlSure.slice(0,140));
      ...
      l('zoom : '+map.getZoom());
      /* etat de la derniere requete de chaque champ vectoriel : c'est ce qui
         permet de distinguer un calque en veille, une requete refusee et un
         trace en echec, sans avoir a ouvrir une console */
      var bV=map.getBounds();
      l('bornes vue : '+bV.getSouth().toFixed(1)+'/'+bV.getNorth().toFixed(1)+' '
        +bV.getWest().toFixed(1)+'/'+bV.getEast().toFixed(1));
      [['vent',window.__dgVent],['pression',window.__dgPres]].forEach(function(p3){
        var g3=p3[1];
        if(!g3){ l('champ '+p3[0]+' : aucune requete depuis le chargement'); return; }
        l('champ '+p3[0]+' : '+g3.etat);
        l('    a '+g3.h+', zoom '+g3.zoom+', maille '+g3.maille);
        l('    zone '+g3.zone);
        if(g3.affichage) l('    '+g3.affichage);
      });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
      ...
      if (p === '/api/admin/diag' && req.method === 'GET') {
        ...
        const resume = [
          ...
          'Mode aisstream : ' + (aisMode === 'zone'
            ...
            : ('filtre MMSI (plafond ' + AIS_PLAFOND + ' impose par le service)'
               + (aisZoneEchec ? ' — zone impossible : ' + aisZoneEchec : '')
               + ' — ' + aisDerniereConnue.size + ' position(s) connue(s) pour calculer la zone')),
          horsPlafond.length
          ...
          'Bateaux de la flotte vus dans le flux : ' + aisVus.size + '/' + suivis
            + (aisMode === 'zone' ? ' (mode zone : un bateau hors boite ne recoit rien)' : ''),
    ...
    /* Champ de pression au niveau de la mer, en grille reguliere : sert au
       trace des isobares et au reperage des centres. Contrairement au vent, la
       pression se lit a l'echelle synoptique : la fenetre autorisee est donc
       bien plus large, et la grille est renvoyee telle quelle (nx, ny et les
       valeurs dans l'ordre des lignes) pour que le client puisse l'interpoler. */
    if (p === '/api/pression/champ' && req.method === 'GET') {
      const pa0 = num(parseFloat(u.searchParams.get('lat0'))), pa1 = num(parseFloat(u.searchParams.get('lat1')));
      const po0 = num(parseFloat(u.searchParams.get('lon0'))), po1 = num(parseFloat(u.searchParams.get('lon1')));
      if (pa0 === null || pa1 === null || po0 === null || po1 === null) return json(res, 400, { error: 'lat0 lat1 lon0 lon1 requis' });
      if (pa1 - pa0 <= 0 || po1 - po0 <= 0 || pa1 - pa0 > 170 || po1 - po0 > 240) return json(res, 400, { error: 'fenetre invalide (max 170 x 240 degres)' });
      let px = parseInt(u.searchParams.get('nx'), 10) || 12;
      let py = parseInt(u.searchParams.get('ny'), 10) || 16;
      px = Math.max(4, Math.min(24, px)); py = Math.max(4, Math.min(30, py));
      while (px * py > 320) { if (py >= px) py--; else px--; }   /* URL Open-Meteo raisonnable */
      ...
      const grille = vals.map((v) => (v === null || !isFinite(v)) ? null : Math.round(v * 10) / 10);
      return json(res, 200, { t: new Date().toISOString(), nx: px, ny: py, lat0: pa0, lat1: pa1, lon0: po0, lon1: po1, valeurs: grille }, req);
    }
    ...
    if (p === '/api/vent/champ' && req.method === 'GET') {
      ...
      if (va1 - va0 <= 0 || vo1 - vo0 <= 0 || va1 - va0 > 170 || vo1 - vo0 > 240) return json(res, 400, { error: 'fenetre invalide (max 170 x 240 degres)' });
      ...
      const cleV = cacheCle('vent', va0, va1, vo0, vo1, nx, ny, modele, heure);
      let e = cacheLire(cleV, 600000);
      if (!e) {
        omDernierMotif = '';
        e = await omGrid(qlat, qlon, modele, heure);
        if (!e || !e.length) return json(res, 502, { error: 'vent indisponible', detail: omDernierMotif || 'sans motif', points_demandes: qlat.length });
        cacheEcrire(cleV, e);
      }
      const pts = [];
      ...
      return json(res, 200, { t: new Date().toISOString(), nx: nx, ny: ny, lat0: va0, lat1: va1, lon0: vo0, lon1: vo1, points: pts }, req);
    }
...
let aisMode = 'mmsi', aisZoneInfo = '', aisHorsFlotte = 0, aisZoneEchec = '';
...
function aisBoiteFlotte() {
  ...
  for (const mmsi of aisInfo.keys()) {
    /* position vue depuis le demarrage, sinon derniere position archivee du
       bateau : sans ce repli, une flotte dont aucun bateau n'a encore emis
       depuis le redemarrage ne pouvait jamais passer en mode zone — or c'est
       precisement le mode qui lui permettrait de recevoir quelque chose. */
    let p = aisLast.get(mmsi);
    if (!p || !isFinite(p[0]) || !isFinite(p[1])) p = aisDerniereConnue.get(mmsi);
  ...
  /* Marge : fixe de 2 deg quand toute la flotte est localisee, elargie quand
     seule une partie l'est — les bateaux sans position connue sont
     vraisemblablement dans les memes eaux, mais rien ne le garantit, et un
     bateau hors boite ne recoit RIEN. On paie donc quelques messages tiers de
     plus plutot que de perdre des bateaux. */
  const couverts = pos.length, total = aisInfo.size;
  const marge = (couverts >= total) ? 2 : Math.min(8, 2 + 6 * (1 - couverts / Math.max(1, total)));
  ...
  return { boite: [[la0, lo0], [la1, lo1]],
           info: Math.round(hauteur) + '\u00b0 x ' + Math.round(largeur) + '\u00b0, ' + couverts + '/' + total
                 + ' positions connues, marge ' + marge.toFixed(1) + '\u00b0' };
}
function aisConnect(force) {
  ...
  /* Garde-fou : ne jamais envoyer plus de MMSI que le plafond du service.
     Au-dela, aisstream ferme la connexion (code 1006) en boucle et plus rien
     n'arrive — y compris pour les bateaux qui tenaient dans la limite. */
  if (filtre && filtre.length > AIS_PLAFOND) filtre = filtre.slice(0, AIS_PLAFOND);
...
async function aisRefresh(reconnect) {
  ...
  /* amorcage des positions connues : une seule lecture par bateau, au refresh,
     pour que le mode zone puisse se calculer des le demarrage */
  try {
    const aAmorcer = Array.from(aisInfo.entries()).filter(([m]) => !aisDerniereConnue.has(m) && !aisLast.has(m));
    for (let i = 0; i < aAmorcer.length; i += 12) {
      const lot = aAmorcer.slice(i, i + 12);
      await Promise.all(lot.map(async ([mmsi, inf]) => {
        try {
          const p = await store.lastPoint(inf.tid);
          if (p && isFinite(p[0]) && isFinite(p[1])) aisDerniereConnue.set(mmsi, [p[0], p[1]]);
        } catch {}
      }));
    }
  } catch {}
