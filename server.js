  ...
  try {
    ...
    if (p.indexOf('/api/admin/') === 0) {
      ...
      if (p === '/api/admin/diag' && req.method === 'GET') {
        const names = Object.keys(process.env).filter((n) => /AIS|OWM|UPSTASH|ADMIN|DATA|PORT/i.test(n));
        return json(res, 200, {
          build: BUILD,
...
let vapiTimer = null, vapiPollMs = 0, vapiLastEvent = 'inactif', vapiPositions = 0, vapiLastAt = 0;
...
async function aisIngest(mmsi, lat, lon, t, sog, cog) {
  ...
  const now = Date.now();
  if (now - (aisLast.get(String(mmsi)) || 0) < info.ms) return false;
  aisLast.set(String(mmsi), now);
...
async function vapiPoll() {
  ...
  for (let i = 0; i < list.length; i += 50) {
    ...
    const url = 'https://api.vesselapi.com/v1/vessels/positions?filter.idType=mmsi&pagination.limit=50&filter.ids=' + chunk.join(',');
    try {
      const r = await fetch(url, { headers: { Authorization: 'Bearer ' + VAPI_KEY } });
      if (!r.ok) { vapiLastEvent = 'HTTP ' + r.status + (r.status === 401 ? ' (cle refusee)' : r.status === 429 ? ' (quota atteint)' : ''); continue; }
      const j = await r.json();
      const rows = Array.isArray(j) ? j : (j.data || j.vessels || j.positions || []);
      let n = 0;
