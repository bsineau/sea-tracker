...
function creerCourantsLayer(map){
  ...
  function fleche(lat, lon, vitesse, dir){
    ...
    var v = Math.min(vitesse, 5);
    /* plancher de 21 px : une fleche de 0,2 kt reste visible ; echelle etiree
       ensuite pour que les ecarts de vitesse restent lisibles */
    var lg = 21 + Math.sqrt(v) * 13;           /* pixels */
    var ep = 2.2 + v * 0.55;
    /* palette contrastee des le premier niveau (fond de carte marine clair) */
    var couleur = v < 0.5 ? '#0369a1' : (v < 1.5 ? '#0284c7' : (v < 3 ? '#ea580c' : '#dc2626'));
    var html = '<div style="transform:rotate(' + dir + 'deg);transform-origin:50% 100%;width:' + lg + 'px;height:' + lg + 'px;position:relative">'
      ...
      + '<path d="M12 23 L12 4" stroke="rgba(255,255,255,.85)" stroke-width="' + (ep + 1.6) + '" stroke-linecap="round" fill="none"/>'
      + '<path d="M6 10 L12 3 L18 10" stroke="rgba(255,255,255,.85)" stroke-width="' + (ep + 1.6) + '" stroke-linecap="round" stroke-linejoin="round" fill="none"/>'
...
const BUILD = '30/07 — fleches de courant v2';
