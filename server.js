...
<div class="wrap">
  ...
  <div id="result" style="display:none">
    <p style="color:var(--green);font-weight:600;margin-top:0">✓ Ton bateau est enregistré dans la flotte.</p>
     ...
     $('leave').onclick=function(){
       ...
       fetch('/api/tracks/'+d.id+'/leave',{method:'POST',headers:{'x-publish-key':d.publishKey}})
        .then(function(r){return r.json();}).then(function(x){
          ...
          $('leaveMsg').textContent=x.retire?'Tu es retiré de la flotte.':'Tu n\\'étais dans aucune flotte.';
          b.style.display='none';
...
async function aisHandle(raw) {
  ...
  if (m && m.error) { aisLastEvent = 'erreur AIS: ' + String(m.error).slice(0, 120); return; }
