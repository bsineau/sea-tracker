...
function creerPressionIsobares(map){
  ...
  function dansCouverture(a0, a1, o0, o1){
    ...
    /* si on a beaucoup zoome, la maille chargee devient trop lache : on recharge */
    return (a1 - a0) / (couv.la1 - couv.la0) > 0.5;
  }
...
function creerVentFleches(map){
  ...
  function dessinerVent(d){
    ...
    /* pas d'affichage : purement visuel, les barbules intermediaires sont
       interpolees et ne coutent aucun appel supplementaire */
    var taille = map.getSize(), PAS = 38;
    var NX = Math.max(4, Math.min(18, Math.round(taille.x / PAS)));
    var NY = Math.max(4, Math.min(30, Math.round(taille.y / PAS)));
    ...
    for(var y = 0; y < NY; y++){
      for(var x = 0; x < NX; x++){
        ...
        barbuleVent(d.lat0 + y * dA, d.lon0 + x * dO, kt2, dir2).addTo(groupe);
      }
    }
  }
...
const BUILD = '01/08o — maille de barbules resserree';
