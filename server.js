...
map.on('click', function(e){
  ...
  fetch('/api/fond?lat='+ll.lat.toFixed(5)+'&lon='+ll.lng.toFixed(5)).then(function(r){return r.json();}).then(function(f){
    if(f&&f.fond!=null){fondHtml='<br>\u26F0 Fond '+(f.source||'')+' : '+f.fond.toFixed(1).replace('.',',')+' m <span style="opacity:.65">('+(f.ref||'')+')</span>';rendre();}
  }).catch(function(){});
...
const BUILD = '26/07 — fond au tap v3 (EMODnet)';
    ...
    if (p === '/api/fond' && req.method === 'GET') {
      ...
      async function interroger(url) {
        ...
        try {
          ...
          for (const v of m) {
            ...
            if (isFinite(n2) && n2 > -200 && n2 < 500 && Math.abs(Math.abs(n2) - 9999) > 1) return Math.round(n2 * 10) / 10;
          }
          return null;
      ...
      if (fond === null) {
        ...
        const qs = 'SERVICE=WMS&VERSION=1.1.1&REQUEST=GetFeatureInfo&SRS=EPSG:4326'
          ...
          + '&LAYERS=emodnet:mean_atlas_land&QUERY_LAYERS=emodnet:mean_atlas_land'
        ...
        if (fond !== null) { source = 'EMODnet'; ref = 'niveau moyen approx.'; }
      }
