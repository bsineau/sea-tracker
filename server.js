...
(function(){
  ...
  function rendre(d){
    ...
    $a('anMsg').textContent=bs.length+' bateau(x) · lissage '+d.fenetreMin+' min'
    ...
    if(!bs.length){$a('anTable').innerHTML='<div style="color:#8fb0c4">Aucune position reçue.</div>';return;}
  ...
  (function(){
    var sp=$a('anPol');
    if(window.POLAIRES_PRESETS) for(var k in POLAIRES_PRESETS){
      var o=document.createElement('option'); o.value=k; o.textContent=POLAIRES_PRESETS[k].n; sp.appendChild(o);
    }
  })();
...
const BUILD = '30/07 — analyse de flotte v2';
          ...
          try {
            ...
            if (dv && dv.current) {
              ...
              for (const L2 of lignes) {
                ...
                if (cible > 0.3) L2.pctPolaire = Math.round(ref / cible * 100);
              }
            }
          } catch { ventInfo = null; }
        }
