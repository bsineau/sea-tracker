...
(function(){
  ...
  function svgPolaire(d){
    ...
    var vmax=0; d.grille.forEach(function(c){ if(c.n>=3 && c.mediane>vmax) vmax=c.mediane; });
    if(vmax<=0) return '<div style="color:#8fb0c4">Pas assez de mesures pour tracer le diagramme.</div>';
    ...
    /* une courbe par TWS */
    var parTws={};
    d.grille.forEach(function(c){ if(c.n<3) return; (parTws[c.tws]=parTws[c.tws]||[]).push(c); });
    ...
    tws.forEach(function(t,i){
      ...
      var col=couleurs[i%couleurs.length], dPath='';
      pts.forEach(function(c,k){
      ...
      s+='<path d="'+dPath+'" fill="none" stroke="'+col+'" stroke-width="2" stroke-linejoin="round"/>';
      pts.forEach(function(c){
        var rad=c.twa*Math.PI/180, r=R*Math.min(c.mediane,vmax)/vmax;
        s+='<circle cx="'+(cx+r*Math.sin(rad)).toFixed(1)+'" cy="'+(cy-r*Math.cos(rad)).toFixed(1)+'" r="2.6" fill="'+col+'"><title>'+c.tws+' kt / '+c.twa+'° : '+c.mediane+' kt ('+c.n+' mesures)</title></circle>';
      });
    ...
    s+='</svg>';
    if(!leg) return '<div style="color:#8fb0c4">Pas encore de tranche de vent avec au moins deux angles mesurés.</div>';
    return s+'<div style="font-size:11px;color:#8fb0c4;margin-top:6px;text-align:center">'+leg+'</div>'
      +'<div style="font-size:11px;color:#8fb0c4;margin-top:4px;text-align:center">Cases à moins de 3 mesures non tracées.</div>';
  }
...
const BUILD = '31/07 — polaire diagramme + periode';
  ...
  try {
    ...
    if (mAnalyse && req.method === 'GET') {
      ...
      for (const bid of membres) {
        ...
        const seg = pts.filter((q) => q[2] >= debut);
        ...
        const ligne = {
          ...
          sogLisse: sogLisse, cmg: cmg, distFenetre: distFenetre, nPoints: seg.length
        };
        if (aMarque) {
          const M = [mLat, mLon];
