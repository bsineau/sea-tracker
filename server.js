  ...
  <div class="card">
    ...
    <div id="triZone" style="display:none">
      <div style="margin:8px 0 6px;display:flex;gap:8px;align-items:center">
        <label style="font-size:12px"><input type="checkbox" id="triTous"> tout cocher</label>
        <span id="triCompte" style="font-size:12px;color:#8fb0c4"></span>
      </div>
      <div id="triListe" style="max-height:300px;overflow:auto;font-size:13px;line-height:1.9"></div>
...
var triCache=[];
  ...
  $('triBtn').onclick=function(){
    ...
    api('/api/admin/diag').then(function(d){
      ...
      $('triCompte').textContent=triCache.length+' bateau(x) désactivé(s)';
      ...
      triCache.forEach(function(x){
        h+='<label style="display:flex;gap:8px;align-items:center"><input type="checkbox" class="triCase" value="'+esc(x.tid||'')+'">'
          +'<span>'+esc(x.nom)+' <span style="color:#8fb0c4">('+esc(x.mmsi)+')</span></span></label>';
      });
  ...
  $('triReactiver').onclick=function(){
    ...
    api('/api/admin/suivi',{method:'POST',body:JSON.stringify({ids:ids,suivi:true})}).then(function(){
  ...
  $('triSupprimer').onclick=function(){
    ...
    (function suivant(){
      ...
      api('/api/admin/boats/'+ids.shift(),{method:'DELETE'}).then(function(){fait++;suivant();}).catch(function(){echecs++;suivant();});
    })();
  };
  } /* fin garde triBtn */
...
const BUILD = '30/07 — compteurs AIS par type';
...
async function aisHandle(raw) {
  ...
  if (!info) { aisRejetInconnu++; aisLastEvent = 'position hors liste (' + mmsi + ')'; return; }
  const now = Date.now();
