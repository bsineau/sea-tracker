...
var weather={};
(function(){
  var owm=function(couche){
    var c=L.tileLayer('https://tile.openweathermap.org/map/'+couche+'/{z}/{x}/{y}.png?appid={cle}',
      {opacity:0.55,maxNativeZoom:12,maxZoom:18,pane:'meteoPane',attribution:'Météo &copy; OpenWeather'});
    var base=c.getTileUrl.bind(c);
    c.getTileUrl=function(coords){ return base(coords).replace('{cle}', encodeURIComponent(window.OWM_KEY||'')); };
    return c;
  };
  weather['Vent']=owm('wind_new'); weather['Pression']=owm('pressure_new');
...
function buildLayerMenu(map, bases, overlays){
  ...
  function addOverlay(layer, name){
    ...
    function appliquer(actif){
      inp.checked=actif;
      if(actif){ if(!map.hasLayer(layer))map.addLayer(layer); map.fire('overlayadd',{layer:layer,name:name}); }
...
const BUILD = '31/07 — bascule calques iOS';
