...
map.on('click', function(e){
  ...
  fetch('/api/fond?lat='+ll.lat.toFixed(5)+'&lon='+ll.lng.toFixed(5)).then(function(r){return r.json();}).then(function(f){
    if(f&&f.fond!=null&&pop&&pop.isOpen&&pop.isOpen()){
      var el=pop.getElement&&pop.getElement();
      var cible=el&&el.querySelector('.leaflet-popup-content');
      if(cible){cible.insertAdjacentHTML('beforeend','<br>\u26F0 Fond Litto3D : '+f.fond.toFixed(1).replace('.',',')+' m <span style="opacity:.65">(alt. NGF)</span>');pop.update();}
    }
  }).catch(function(){});
...
const BUILD = '26/07 — Litto3D littoral + fond au tap';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/fond' && req.method === 'GET') {
      ...
      const d = 0.0015; /* ~150 m de fenetre */
      const bbox = [clat - d, clon - d, clat + d, clon + d].join(','); /* WMS 1.3.0 : lat,lon */
      const couches = LITTO3D_COUCHES.join(',');
      const qs = 'SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo&CRS=EPSG:4326'
        + '&BBOX=' + bbox + '&WIDTH=101&HEIGHT=101&I=50&J=50'
        + '&LAYERS=' + couches + '&QUERY_LAYERS=' + couches
        + '&STYLES=&FORMAT=image/png&INFO_FORMAT=' + encodeURIComponent('text/plain')
        + '&FEATURE_COUNT=5';
      let fond = null;
      try {
        const ac = new AbortController();
        const tm = setTimeout(() => ac.abort(), 6000);
        const rep = await fetch('https://services.data.shom.fr/INSPIRE/wms/r?' + qs, { signal: ac.signal });
        clearTimeout(tm);
        const txt = await rep.text();
        /* la reponse texte contient la valeur du pixel ; on prend le premier
           nombre plausible (metres, -100..500) hors valeurs de non-donnee */
        const m = txt.match(/-?\d+(?:[.,]\d+)?/g) || [];
        for (const v of m) {
          const n2 = parseFloat(v.replace(',', '.'));
          if (isFinite(n2) && n2 > -100 && n2 < 500 && Math.abs(n2 + 9999) > 1 && Math.abs(n2 - 9999) > 1) { fond = Math.round(n2 * 10) / 10; break; }
        }
      } catch {}
      return json(res, 200, { fond: fond, ref: 'NGF-IGN69' });
    }
