...
var overlays=Object.assign({'Balises (OpenSeaMap)':seamark,'Balises SHOM':shomBalise,'Relief fonds Litto3D (Shom)':creerLitto3D(),'Vent animé (Open‑Meteo)':windGroup},weather);
...
<div id="rtPanneau" style="display:none;position:fixed;right:14px;top:calc(env(safe-area-inset-top) + 128px);z-index:1500;background:#0d1f2dee;border:1px solid #1d3a52;border-radius:14px;padding:12px;width:250px;color:#dfeaf2;font-size:12.5px;line-height:1.6">
  ...
  <label style="display:block;margin:0 0 6px;font-size:11px;color:#8fb0c4">Charger une polaire (.pol)<input id="rtPolFichier" type="file" accept=".pol,.txt,.csv,.tsv" style="display:block;margin-top:3px;width:100%"></label>
  ...
  <label style="display:block;margin-top:6px;font-size:11px;color:#8fb0c4">Zones interdites (GPX)<input id="rtGpx" type="file" accept=".gpx" style="display:block;margin-top:3px;width:100%"></label>
...
(function(){
  ...
  $r('rtGo').onclick=function(){
    $r('rtMsg').textContent='';
    rtLancer({pctPolaire:parseInt($r('rtPct').value,10),maxHeures:48,pasMinutes:12,pasCap:6},
      ...
      function(m2){rtDessiner(m2);
        ...
        $r('rtMsg').textContent=(m2.arriveeAtteinte?'Arrivée ':'Au plus près (reste '+m2.distRestante+' MN) ')
          +hh+' ('+duree.toFixed(1)+' h) · '+RT.polaireNom;},
...
function renderLegend(){
  ...
  var head='<span id="lgtog" style="cursor:pointer;color:#39c0d3;font-weight:700">'+(legOpen?'▾ masquer':'▸ afficher')+'</span> · '+ks.length+' bateau'+(ks.length>1?'x':'');
...
const BUILD = '26/07 — routeur polaire chargeable';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
      ...
      if (p === '/api/admin/diag' && req.method === 'GET') {
        ...
        return json(res, 200, {
          ...
          aisDetail: (function () {
            ...
            for (const x of aisExclus) out.push({ mmsi: x.mmsi, nom: x.nom, desactive: true });
            return out;
          })(),
    ...
    if (p === '/api/routeur/grilles' && req.method === 'GET') {
      ...
      if (la1 - la0 <= 0 || lo1 - lo0 <= 0 || la1 - la0 > 6 || lo1 - lo0 > 8) return json(res, 400, { error: 'domaine invalide (max 6 x 8 degres)' });
      const modele = (u.searchParams.get('modele') || 'best_match').replace(/[^a-z_0-9]/g, '');
      const heures = Math.min(72, Math.max(12, parseInt(u.searchParams.get('heures'), 10) || 48));
      /* pas de grille : viser <= 18 points par axe */
      const pasLat = Math.max(0.05, (la1 - la0) / 17), pasLon = Math.max(0.05, (lo1 - lo0) / 17);
      const lats = [], lons = [];
      ...
      const cle = ['rg', lats[0], lats[lats.length-1], lons[0], lons[lons.length-1], modele, heures, Math.floor(Date.now() / 3600e3)].join('|');
      ...
      try {
        for (let i = 0; i < ptsG.length; i += 90) {
          ...
          const url = 'https://api.open-meteo.com/v1/forecast?latitude=' + lot.map((q) => q[0]).join(',')
            ...
            + '&forecast_days=' + Math.ceil(heures / 24 + 0.5) + '&timezone=UTC'
          ...
          for (let k2 = 0; k2 < tab.length; k2++) {
            ...
            if (!h2) continue;
            if (!tempsVent) tempsVent = h2.time.slice(0, heures + 1);
            vent[i + k2] = { s: h2.wind_speed_10m.slice(0, heures + 1).map((x2) => x2 === null ? null : Math.round(x2 * 10) / 10),
                             d: h2.wind_direction_10m.slice(0, heures + 1).map((x2) => x2 === null ? null : Math.round(x2)) };
          }
        }
      ...
      if (C2D) {
        ...
        const coefParH = [];
        for (let ih = 0; ih <= heures; ih++) coefParH.push(await coefficientA(t0g + ih * 3600e3));
        const refParPortH = new Map();
        for (const [cleP, pp] of phases) {
          const tabRef = [];
          for (let ih = 0; ih <= heures; ih++) tabRef.push(await phasePour(pp.port, pp.base, t0g + ih * 3600e3));
        ...
        for (let ip = 0; ip < ptsG.length; ip++) {
          ...
          const su = [], sv2 = [];
          for (let ih = 0; ih <= heures; ih++) {
            const ref = tabRef && tabRef[ih], coefH = coefParH[ih];
            if (!ref || coefH === null) { su.push(null); sv2.push(null); continue; }
            const hRel = Math.max(-6, Math.min(6, (t0g + ih * 3600e3 - ref.t) / 3600e3));
      ...
      const sortieG = { lats, lons, temps: tempsVent, modele, vent, courant, t0: t0g };
...
let vapiTimer = null, vapiPollMs = 0, vapiLastEvent = 'inactif', vapiPositions = 0, vapiLastAt = 0, vapiBrut = '', vapiPauseJusqua = 0;
...
function aisConnect(force) {
  ...
  ws.onopen = () => {
    ...
    const sub = { APIKey: AIS_KEY, BoundingBoxes: [[[-90, -180], [90, 180]]], FiltersShipMMSI: list };
...
async function vapiPoll() {
  const list = Array.from(aisInfo.keys());
  ...
  for (let i = 0; i < list.length; i += 50) {
    ...
    for (let garde = 0; garde < 10; garde++) {
      let url = 'https://api.vesselapi.com/v1/vessels/positions?filter.idType=mmsi&pagination.limit=100'
      ...
      try {
        ...
        if (!r.ok) {
          erreur = 'HTTP ' + r.status + (r.status === 401 ? ' (cle refusee)' : (r.status === 429 || r.status === 403) ? ' (quota atteint)' : r.status === 400 ? ' (requete refusee)' : '');
          if (r.status === 429 || r.status === 403) {
            let sec = 3600;
            const m = brut.match(/Retry after (\d+) seconds/i); if (m) sec = Math.min(43200, parseInt(m[1], 10) + 60);
            vapiPauseJusqua = Date.now() + sec * 1000;
            erreur += ' — pause ' + Math.round(sec / 60) + ' min';
          }
          break;
        }
...
async function aisRefresh(reconnect) {
  ...
  for (let k = 0; k < mmsis.length; k++) {
    ...
    if (meta.suivi === false) { aisExclus.push({ mmsi: mmsi, nom: meta.name || ('MMSI ' + mmsi) }); continue; }
