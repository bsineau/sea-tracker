...
function creerVentFleches(map){
  ...
  function dessinerVent(d){
    ...
    for(var y = 0; y < NY; y++){
      for(var x = 0; x < NX; x++){
        ...
        flecheVent(d.lat0 + y * dA, d.lon0 + x * dO, kt2, dir2).addTo(groupe);
      }
    }
  }
  ...
  function corpsLegV(){
    ...
    return '<div style="font-weight:700;margin-bottom:3px">Vent (kt)</div>'
      ...
      + '<div style="font-size:10px;color:#8fb0c4">fleche = sens du flux</div>';
  }
  function veilleLegV(){
    if(elV) elV.innerHTML = '<div style="font-weight:700;margin-bottom:2px">Vent (kt)</div>'
      + '<div style="font-size:10px;color:#8fb0c4">fleches a partir du zoom 2</div>';
  }
...
const BUILD = '01/08m — interpolation + marge de couverture';
